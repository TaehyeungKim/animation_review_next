    ...
    return (
        opened ? <div className={styles.colorPalette} style={styleOfTheSubPalette} ref={colorPaletteRef}>
            {colorArr.map((color: string, index: number) => {
                ...
                return <ButtonComponent className={`${styles.color_button} ${styles['color_button--animation']}`} key={index} event={[['click', change.bind(null, selection, {
                    propertyKey: 'color',
                    propertyValue: color
                })]]} style={{animationDelay: `${(index+1) * 30}ms`}}>{colorPaletteElm}</ButtonComponent>
            })
            }
        </div> : null
    )
...
function TextEditPalette({change, rangeDomRect}: TextEditPaletteProps) {
    ...
    return(
        <div className = {styles.container} contentEditable='false' suppressContentEditableWarning ref={containerRef} style={paletteStyle as React.CSSProperties} id="texteditpalette">
            <ButtonComponent className = {styles.colors} event={[['click', showSubComponent.bind(null, "color")],['select', ()=>{return false}]]}>
                {fontColor()}
            </ButtonComponent>
            <ButtonComponent className={styles.bold} event={[['click', change.bind(null, selection, {
